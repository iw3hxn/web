...
openerp.web.Database = openerp.web.OldWidget.extend(/** @lends openerp.web.Database# */{
    ...
    start: function() {
        ...
        this.$element.find('#db-duplicate').click(this.do_duplicate);
    ...
    stop: function () {
        ...
        this.$element
            .find('#db-create, #db-drop, #db-backup, #db-restore, #db-duplicate, #db-change-password, #back-to-login')
                .unbind('click')
            .end()
            .empty();
        this._super();
    ...
    do_duplicate: function() {
        var self = this;
       	self.$option_id.html(QWeb.render("DuplicateDB", self));
        
        self.$option_id.find("form[name=duplicate_db_form]").validate({
            submitHandler: function (form) {
                self.blockUI();
                $(form).ajaxSubmit({
                    ...
                    success: function (body) {
                        ...
                        if (body.indexOf('403 Forbidden') !== -1) {
                            ...
                            self.display_error({
                                ...
                                error: 'Could not duplicate the database'
                            })
                        }
                    },
                    complete: function() {
                        self.unblockUI();
                        self.do_notify(_t("Duplicated"), _t("Database duplicate successfully"));
                    }
                });
            }
        });
    ...
    do_update: function () {
        ...
        var fct = function() {
            ...
            return func(self.session.uid, ["name", "company_id"]).pipe(function(res) {
                ...
                self.$element.find("a.request").click(self.on_request);
